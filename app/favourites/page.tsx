...
import { useFavourites, type FavouriteItem } from "@/app/hooks/useFavourites";
import { IconHeartOff, IconArrowRight } from "@tabler/icons-react";

function getInitialItems(getAll: () => FavouriteItem[]): FavouriteItem[] {
  if (typeof window === "undefined") return [];
  return getAll();
}

export default function FavouritesPage() {
  const { getAll } = useFavourites();
  const [items, setItems] = useState<FavouriteItem[]>(() => getInitialItems(getAll));
      ...
      <main className="page-container py-4 xs:py-6 sm:py-8 md:py-10 lg:py-12">
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <IconHeartOff className="w-16 h-16 text-muted-foreground mb-4" stroke={1} />
            <h2 className="text-xl font-semibold mb-2">No favourites yet</h2>
            <p className="text-foreground-secondary mb-6">
              Favourite movies, TV shows, and anime to find them here quickly.
            </p>
